    ...
    return (
        <div style={{ marginTop: "160px" }}>
            ...
            <div>
                <h2>Repositories</h2>
                {repos.map((repo) => (
                    <div key={repo.id} onClick={() => fetchContents(repo.name)}>
                        📁 {repo.name}
                    </div>
                ))}
            </div>
            ...
            <div>
                <h2>Contents</h2>
                {contents.map((item) => (
                    <div key={item.sha} onClick={() => item.type === 'file' ? fetchFileContent(currentRepo, item.path) : fetchContents(currentRepo, item.path)}>
